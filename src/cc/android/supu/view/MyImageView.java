...
public class MyImageView extends ImageView {
	...
	private float imageWidth;// 图片的原始宽度
	private float imageHeight;// 图片的原始高度
	...
	protected void onDraw(Canvas canvas) {
		...
		if (width > ProductBigImageActivity.screenWidth) {
			// 如果图宽大于屏宽，就不用水平居中
			center(false, false);
		} else {
			center(true, true);
		}
		if (height > ProductBigImageActivity.screenHeight) {
			center(false, false);
		} else {
			center(true, true);
		}
	...
	public MyImageView(Context context, int imageWidth, int imageHeight) {
		...
		this.imageWidth = metrics.widthPixels;
		this.imageHeight = metrics.heightPixels;
		init();
	}
	...
	public MyImageView(Context context, AttributeSet attrs) {
		...
		if (AsyncImageLoader.isShow) {
			if (metrics.heightPixels < 500) {
				...
				this.imageWidth = 3 * metrics.widthPixels;
				this.imageHeight = 3 * metrics.heightPixels;
			} else if (metrics.heightPixels < 900) {
				System.out.println("2222");
				this.imageWidth = 2 * metrics.widthPixels;
				this.imageHeight = 2 * metrics.heightPixels;
			} else {
				System.out.println("3333");
				this.imageWidth = (float) (1.2 * metrics.widthPixels);
				this.imageHeight = (float) (1.2 * metrics.heightPixels);
			}
		} else {
			this.imageWidth = metrics.widthPixels;
			this.imageHeight = metrics.heightPixels;
	...
	private void arithScaleRate() {
		float scaleWidth = ProductBigImageActivity.screenWidth
				/ (float) imageWidth;
		float scaleHeight = ProductBigImageActivity.screenHeight
				/ (float) imageHeight;
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		arithScaleRate();
		// 缩放到屏幕大小
		zoomTo(scaleRate, ProductBigImageActivity.screenWidth / 2f,
				ProductBigImageActivity.screenHeight / 2f);
	...
	public void layoutToCenter() {
		...
		// 空白区域宽高
		float fill_width = ProductBigImageActivity.screenWidth - width;
		float fill_height = ProductBigImageActivity.screenHeight - height;
	...
	protected float getValue(Matrix matrix, int whichValue) {
		...
		mMinZoom = (ProductBigImageActivity.screenWidth / 2f) / imageWidth;
