...
public class SettleAccountActivity extends BaseActivity {
	...
	protected void enterBtnOnClick() {
		...
		if (consigneeBean == null) {
			Toast.makeText(context, "您的收件信息为空，请先完善收件信息", Toast.LENGTH_SHORT)
					.show();
		...
		if (!"".equals(allCashStr)
				&& !allCashStr.matches("([0-9]{1,})|([0-9]{1,}[.][0-9]{1,2})")) {
			cashEt.requestFocus();
			Toast.makeText(context, "现金账户金额输入格式不正确！", Toast.LENGTH_SHORT)
					.show();
			return;
		}
		...
		if (PriceTools.compare(allCash, price) == 1) {
			cashEt.requestFocus();
			Toast.makeText(context, "您输入的现金账户金额不能大于您账户现有的总额！",
		...
		if (PriceTools.compare(allCash, cashAmount) == 1) {
			System.out.println("payMoney="
					+ PriceTools.formatStr(getPayMoney(sumAmount, shippingFee,
							ticketDiscount, "0.0")));
			payMoneyTv.setText("￥"
					+ PriceTools.formatStr(getPayMoney(sumAmount, shippingFee,
							ticketDiscount, "0.0")));
			cashEt.setText("");
			cashEt.requestFocus();
			Toast.makeText(context, "您输入的现金账户金额不能大于您需要支付的金额！",
					Toast.LENGTH_SHORT).show();
			return;
		}

		if (paymentList == null || paymentList.size() == 0) {
			Toast.makeText(context, "支付方式为空，请选择支付方式！", Toast.LENGTH_SHORT)
					.show();
			return;
		}
		if (shippingList == null || shippingList.size() == 0) {
			Toast.makeText(context, "配送方式为空，请选择配送方式！", Toast.LENGTH_SHORT)
					.show();
			return;
		}
		if (shippingFee == null || shippingFee.equals("")) {
			Toast.makeText(context, "运费请求失败，请重新选择配送方式或支付方式！",
					Toast.LENGTH_SHORT).show();
			return;
		}
	...
	protected void onSubActivityClick(View view) {
		...
		case R.id.arrow5:
			if (ifDiscount) {
				...
				ticketDiscount = "0";
				payMoneyTv.setText("￥"
						+ PriceTools.formatStr(getPayMoney(sumAmount,
								shippingFee, ticketDiscount, allCash + "")));
				discountTv.setText("￥"
						+ PriceTools.formatStr(getDiscountAmount(
	...
	protected void initPage() {
		...
		((RelativeLayout) findViewById(R.id.settleaccount_invoiceLayout))
		...
		if (getIntent() != null) {
			...
			discountTv.setText("￥"
					+ PriceTools.formatStr(getDiscountAmount(discountAmount,
							ticketDiscount)));
	...
	PageRequest requestDefaultConsignee = new PageRequest() {
		...
		public void requestServer() {
			...
			Tools.requestToParse(context, ConstantUrl.GETDEFAULTCONSIGNEE,
					"GetDefaultConsignee", null, defaultConsigneeHandler, false);
	...
	PageRequest requestPaymentShipping = new PageRequest() {
		...
		public void requestServer() {
			...
			Tools.requestToParse(context, ConstantUrl.GETPAYMENTSHIPPING,
					"GetPaymentShipping", params, paymentShippingHandler, false);
			if (Tools.responseValue == 1) {
	...
	PageRequest requestShippingFee = new PageRequest() {
		...
		public void requestServer() {
			...
			Tools.requestToParse(context, ConstantUrl.GETSHIPPINGFEE,
					"GetShippingFee", params, shippingFeeHandler, false);
			System.out.println("requestShippingFee.........................");
			if (Tools.responseValue == 1) {
				System.out
						.println("responseValue == 1.........................");
				if (shippingFeeHandler.result_code == 0) {
					System.out
							.println("shippingFeeHandler.result_code==0............");
	...
	private PageRequest requestSubmitOrder = new PageRequest() {
		...
		public void requestServer() {
			// TODO Auto-generated method stub
			// String allCash =
			// cashAccountTv.getText().toString().trim().substring(1);
	...
	private PageRequest requestMemberInfo = new PageRequest() {
		...
		public void requestServer() {
			...
			Tools.requestToParse(context, ConstantUrl.GETMEMBERINFO,
					"GetMemberInfo", null, memberInfoHandler, false);
			if (Tools.responseValue == 1) {
	...
	private PageRequest requestTicketInfo = new PageRequest() {
		...
		public void requestServer() {
			...
			Tools.requestToParse(SettleAccountActivity.this,
					ConstantUrl.GETTICKETINFO, "GetTicketInfo", params,
					ticketInfoHandler, false);
			if (Tools.responseValue == 1) {
				if (ticketInfoHandler.result_code == 0) {
					handler.sendMessage(handler
							.obtainMessage(STATE_SUCCESS_TICKETINFO));
				} else {
					handler.sendMessage(handler
							.obtainMessage(STATE_FAILURED_TICKETINFO));
				}
			} else {
				handler.sendMessage(handler
						.obtainMessage(STATE_ERROE_TICKETINFO));
			}
		}
	};
	...
	protected void dealwithMessage(android.os.Message msg) {
		...
		case STATE_FAILURED_PAYMENTANDSHIPPIN:
			...
			if (result_code2 == 1000) {
				...
				Toast.makeText(context, "地区编号为空！请先设置收货地址。", Toast.LENGTH_SHORT)
						.show();
			} else if (result_code2 == 13) {
				Toast.makeText(context, "县/区编号错误！请正确设置收货地址。",
						Toast.LENGTH_SHORT).show();
			} else {
				// Toast.makeText(context, "服务器异常！", Toast.LENGTH_SHORT).show();
			}
		...
		case STATE_SUCCESS_SHIPPINGFEE:
			...
			System.out.println("shippingFeeTv="
					+ shippingFeeTv.getText().toString());
			// ...................................................................
			String cashStr = cashEt.getText().toString().trim();
			if (cashStr.matches("([0-9]{1,})|([0-9]{1,}[.][0-9]{1,2})")
					|| "".equals(cashStr)) {
				allCash = PriceTools.formatStr(cashStr);
				String cashAmount = getPayMoney(sumAmount, shippingFee,
						ticketDiscount, "");
				...
				payMoneyTv.setText("￥"
						+ PriceTools.formatStr(getPayMoney(sumAmount,
								shippingFee, ticketDiscount, allCash + "")));
				// // 现金账户 >输入金额 && 共需支付 > 输入金额
				// if (PriceTools.compare(price, allCash) >= 0 &&
				// PriceTools.compare(maxAmount, allCash) >= 0) {
				// // cashAccountTv.setText("￥" +
				// PriceTools.formatStr(allCash));
				// payMoneyTv.setText("￥" +
				// PriceTools.formatStr(getPayMoney(sumAmount, shippingFee,
				// ticketDiscount, allCash + "")));
				...
				// // }
				// // 现金账户 <输入金额 && 共需支付 > 现金账户
				// } else if (PriceTools.compare(price, allCash) < 0 &&
				// PriceTools.compare(maxAmount, price) >= 0) {
				...
				// PriceTools.compare(maxAmount, price) < 0) {
				// // cashAccountTv.setText("￥" +
				// PriceTools.formatStr(maxAmount));
		...
		case STATE_FAILURED_SHIPPINGFEE:
			...
			if (result_code3 == 1) {
				Toast.makeText(context, "地址为空！请先设置收货地址。", Toast.LENGTH_SHORT)
						.show();
			} else if (result_code3 == 2) {
				Toast.makeText(context, "省为空！请先设置收货地址。", Toast.LENGTH_SHORT)
						.show();
		...
		case STATE_FAILURED_SUBMITORDER:
			...
			if (result_code == 1000) {
				...
				Toast.makeText(context, showText(result_code),
						Toast.LENGTH_SHORT).show();
			}
		...
		case STATE_SUCCESS_GETMEMBERINFO:
			if (memberInfoHandler.memberBean != null) {
				price = PriceTools
						.formatStr(memberInfoHandler.memberBean.price);
		...
		case STATE_SUCCESS_TICKETINFO:
			if ("可使用此优惠券".equals(ticketInfoHandler.validateResult)) {
				...
				Toast.makeText(context, "您所使用的优惠券无效！", Toast.LENGTH_SHORT)
						.show();
			}
			break;
		case STATE_FAILURED_TICKETINFO:
			if (null != ticketInfoHandler.result_message) {
				Toast.makeText(context, ticketInfoHandler.result_message,
						Toast.LENGTH_SHORT).show();
				enterLabel.setBackgroundResource(R.drawable.enter_normal);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (data != null) {
			...
			if (resultCode == 20) {
				...
				TicketBean ticketBean = (TicketBean) data
						.getSerializableExtra("ticketBean");
				if (ticketBean != null) {
					ifDiscount = true;
					...
					payMoneyTv
							.setText("￥"
									+ PriceTools.formatStr(getPayMoney(
	...
	private void showDialog() {
		...
		dialog.setTitle("提示对话框").setMessage("您未登录，请先登录！")
	...
	private TextWatcher watcher = new TextWatcher() {
		...
		public void afterTextChanged(Editable s) {
			...
			if (cashStr.length() >= 2 && cashStr.substring(0, 2).equals("00")) {
				Toast.makeText(SettleAccountActivity.this, "输入的数字错误或超限",
						Toast.LENGTH_SHORT).show();
				cashEt.setText("");
				return;
			}
					...
					if (PriceTools.compare(price, allCash) >= 0
						...
						payMoneyTv.setText("￥"
								+ PriceTools.formatStr(getPayMoney(sumAmount,
										shippingFee, ticketDiscount, allCash
												+ "")));
						String total = getTotalAmount(sumAmount,
								ticketDiscount, allCash + "");
						// if (PriceTools.compare(total, "0") >= 0) {
						// totalCountTv.setText("￥"
						// + getTotalAmount(sumAmount, ticketDiscount,
						...
						mcash = PriceTools.formatStr(allCash);
						cashAccountTv
								.setText("￥" + PriceTools.formatStr(price));
						payMoneyTv.setText("￥"
								+ PriceTools
										.formatStr(getPayMoney(sumAmount,
												shippingFee, ticketDiscount,
												price + "")));
						// String total = getTotalAmount(sumAmount,
						// ticketDiscount,
						// price + "");
						...
						cashAccountTv.setText("￥"
								+ PriceTools.formatStr(maxAmount));
						payMoneyTv.setText("￥"
								+ PriceTools.formatStr(shippingFee));
						// totalCountTv.setText("￥0.00");
						// 现金账户 >输入金额 && 共需支付 < 输入金额
					} else if (PriceTools.compare(price, allCash) >= 0
							&& PriceTools.compare(maxAmount, allCash) < 0) {
	 ...
	 * @return 共需支付的金额
	 */
	private String getPayMoney(String sumAmount, String shippingFee,
			String ticketDiscount, String cashAmount) {
		double payM = 0d;
		...
		if (payc >= 0) {
			...
			mcash = PriceTools.formatStr(allCash);
			cashAccountTv.setText("￥"
					+ PriceTools.formatStr(PriceTools.intToString(cash)));
		} else {
			payM = shipFee;
			mcash = PriceTools.formatStr(allCash);
			cashAccountTv
	 ...
	 */
	private String getTotalAmount(String sumAmount, String discountAmount,
			String cashAmount) {
		double sum = PriceTools.stringToInt(sumAmount);
		double dis = PriceTools.stringToInt(discountAmount);
		double cash = PriceTools.stringToInt(cashAmount);
