...
public class MyGallery extends Gallery {
		...
		this.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				View view = MyGallery.this.getSelectedView().findViewById(
						R.id.bigImageView);
				// System.out.println("kkk:" + (view == null));
				if (view == null)
					return false;
				else
				// System.out.println("---kkk:" + (view instanceof
				// MyImageView));
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
		...
		if (view instanceof MyImageView) {
			...
			if ((int) width <= ProductBigImageActivity.screenWidth
					&& (int) height <= ProductBigImageActivity.screenHeight)
			// 如果图片当前大小<屏幕大小，直接处理滑屏事件
			{
				...
				if (distanceX > 0)// 向左滑动
				{
					if (r.left > 0) {// 判断当前ImageView是否显示完全
						super.onScroll(e1, e2, distanceX, distanceY);
					} else if (right < ProductBigImageActivity.screenWidth) {
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
				...
				{
					if (r.right < ProductBigImageActivity.screenWidth) {
						super.onScroll(e1, e2, distanceX, distanceY);
					} else if (left > 0) {
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (view instanceof MyImageView) {
				...
				if ((int) width <= ProductBigImageActivity.screenWidth
						&& (int) height <= ProductBigImageActivity.screenHeight)// 如果图片当前大小<屏幕大小，判断边界
				{
					break;
				}
				float v[] = new float[9];
				...
				// if (bottom < ProductBigImageActivity.screenHeight) {
				// imageView
				// .postTranslateDur(
				// ProductBigImageActivity.screenHeight
				// - bottom, 200f);
				// }
			}
			break;
			...
			if (view instanceof MyImageView) {
				...
				if (imageView.getScale() > imageView.getScaleRate()) {
					imageView.zoomTo(imageView.getScaleRate(),
							ProductBigImageActivity.screenWidth / 2,
							ProductBigImageActivity.screenHeight / 2, 200f);
